...
Status: **Level 0** (buildable prototype). Tranche 1 baseline infrastructure is accepted. Tranche 2 homogeneous implementation is **open** ([`decisions/2026-08-16-t2-homogeneous.md`](decisions/2026-08-16-t2-homogeneous.md)) and is **not accepted**. There is no LLMP scaling claim. AFL++ remains smoke-only (`t1_compare.sh`). Public positioning until evidence exists:
...
| **achlys-core** | `FuzzerBuilder` wraps LibAFL havoc. `run()` is the CLI path (blackbox). `run_substrate()` is the H0/T1 in-process graybox worker. `CorpusAuthority` / `CandidateSpool` are the T2 control plane (not on the hot path). `CampaignStore` / `CampaignSession` write campaign artifacts. |
...
There is no QEMU backend, no network target, no symbolic engine, and no adaptive orchestrator. Homogeneous multi-worker / LLMP (`achlys_t2`) is implementation-open and not accepted; published scaling is not claimed. Canonical replay and sanitizer verification are not on the per-execution hot path.
...
| `achlys_t2` | Homogeneous multi-worker example. Implementation open; not a T2 accept; no scaling claim. |
